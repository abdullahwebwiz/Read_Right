...
router.post("/adduser", (req, res) => {
  ...
  let userid = ang.generate({ length: 11, charset: "alphabetic" });
  ...
  db1.run(
    `INSERT INTO users (userid,name,email,password,country,city) VALUES
  ('${userid}','${name}','${email}','${password}','${country}','${city}')`,
    (err) => {
      if (err) {
        res.send({ msg: "failed" });
